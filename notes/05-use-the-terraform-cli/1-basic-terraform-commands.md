...
The `terraform validate` command checks whether your configuration syntax is correct and internally consistent. When your configuration is valid, it returns a success message. For example, a valid configuration like the one below will pass the validation check.
...
The `terraform show` command provides a human-readable output of the state file, giving you a detailed view of your infrastructure. This includes a detailed view of resources and their attributes. To facilitate further processing, you can append the `--json` flag, which outputs the state in JSON format for easier integration with other tools.
...
The `terraform output` command reads the state file and lists all output values. This is useful for displaying important information, such as IP addresses or DNS names, after applying your configuration.
...
"[root] provider[\"registry.terraform.io/hashicorp/aws\"]" [label = "provider[\"registry.terraform.io/hashicorp/aws\"]", shape = "box"]
...
Use the `terraform state list` command to display all resources recorded in the state file. The output lists resource addresses in the format `resource_type.resource_name`.
...
resource "aws_s3_bucket" "terraform-state" {
    ...
    bucket_regional_domain_name = "cerberus-finance.s3.eu-west-2.amazonaws.com"
    force_destroy = false
    ...
...
To rename this resource from `state-locking` to `state-locking-db`, you would run:

```bash
$ terraform state mv aws_dynamodb_table.state-locking aws_dynamodb_table.state-locking-db
```

Then, update your configuration file to reflect the new name:
...
The `terraform state pull` command downloads the remote state file to your local directory. This allows you to inspect the state file or make backups.
...
```bash
$ terraform state pull | jq '.resources[] | select(.name == "state-locking-db") | .instances[0].attributes.hash_key'
"LockID"
```
...
Failed to write state: cannot import state with lineage "1dc19ee8-2b7f-d87a-4786-4be724b24988" over remote state with lineage "b6e2cf0e-ef8d-3c59-1e11-c6520dcd745c"
```

Although the `--force` option exists, it is recommended only as a last resort.

That concludes our lesson on basic Terraform commands. You will be better equipped to validate your configurations, maintain a clean state, and manage your infrastructure efficiently.
